...
// .wrangler/tmp/pages-b4e5FU/functionsWorker-0.5850391524818706.mjs
...
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    const API_URL = context.env.NEWEBPAY_ENV === "production" ? "https://core.newebpay.com/MPG/mpg_gateway" : "https://ccore.newebpay.com/MPG/mpg_gateway";
    const orderNo = `ORD_${Date.now()}_${userId.substring(0, 5)}`;
    const amount = priceId === "price_premium" ? 399 : 100;
    ...
    const tradeData = {
      ...
      Amt: amount,
      ItemDesc: "Lumen Tarot Premium",
      // 商品名稱
      Email: email,
      LoginType: 0,
      // 0: 不需登入藍新會員
      // 設定回調網址 (請替換為你的真實網域名稱)
      ReturnURL: `${new URL(context.request.url).origin}/payment/return`,
      // 支付完成跳轉頁
      NotifyURL: `${new URL(context.request.url).origin}/api/payment/notify`,
      // 背景 Webhook
      ClientBackURL: `${new URL(context.request.url).origin}/pricing`
      // 取消返回頁
    };
...
var onRequestPost2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const apiKey = env2.GEMINI_API_KEY || env2.VITE_GEMINI_API_KEY;
  if (!apiKey) {
  ...
  try {
    ...
    const geminiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.0-flash:generateContent?key=${apiKey}`;
    const payload = {
      contents: [{
...
// .wrangler/tmp/bundle-bQSEnW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bQSEnW/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5850391524818706.js.map
